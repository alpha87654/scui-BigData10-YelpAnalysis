...
def auto_chart(df: pd.DataFrame, question: str):
    ...
    try:
        if any(w in q for w in ["distribution", "proportion", "percentage", "ratio"]):
            fig = px.pie(df.head(10), names=x, values=y, title=f"Distribution of {y}")
        elif any(w in q for w in ["year", "month", "trend", "over time", "growth"]):
            fig = px.line(df, x=x, y=y, title=f"{y} over {x}", markers=True)
        else:
            fig = px.bar(df.head(20), x=x, y=y, title=f"{y} by {x}", color=y, color_continuous_scale="Blues")
...
for msg in st.session_state.messages:
    if msg["role"] == "user":
        ...
        if "sql" in msg:
            st.markdown(f'<div class="sql-block">📋 Generated SQL:\n\n{msg["sql"]}</div>', unsafe_allow_html=True)
